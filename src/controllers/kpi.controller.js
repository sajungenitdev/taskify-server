...
const findUsersByDepartment = async (departmentId, departmentName) => {
  ...
  const allUsers = await User.find({ isActive: true })
    .select("_id fullName email department departmentId");

  console.log(`🔍 Total active users: ${allUsers.length}`);

  // Filter manually with more flexible matching
  const matchedUsers = allUsers.filter(user => {
    // Check direct departmentId field
...
const calculateKPIScores = async (req, res) => {
  ...
  try {
    ...
    for (const employee of employees) {
      ...
      // Check if score already exists - ✅ monthStr is now defined
      ...
      if (existingScore) {
        ...
        // Create new score - ✅ monthStr is now defined
...
// ============================================================
// GET MONTHLY KPI REPORT - COMPLETE FIXED VERSION
// ============================================================
const getMonthlyKPIReport = async (req, res) => {
  try {
    ...
    if (!month || !year) {
      ...
      // Only add department filter if provided and valid
      ...
      // If no scores found with department filter, try without filter
...
// ============================================================
// NEW: GET EMPLOYEE KPI (Dashboard)
// ============================================================
...
const getKPILeaderboard = async (req, res) => {
  try {
    ...
    const scores = await KPIScore.find(query)
      .populate("userId", "fullName email employeeId role profilePhoto")
      .populate("departmentId", "name code")
      .sort({ totalScore: -1 })
